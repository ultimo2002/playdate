...
def register_endpoints(app):
    ...
    @app.get("/apps")
    def read_apps(db=db_dependency, all_fields: bool = False, target_name: str = None, like: str = None):
        if target_name:
    ...
    @app.get("/cats")
    def read_cats(db=db_dependency):
    ...
    @app.get("/tags")
    def read_tags(db=db_dependency):
        tags = db.query(models.Tags).all()
        return tags
    ...
    @app.get("/genres")
    def read_genres(db=db_dependency):
        genres = db.query(models.Genre).all()
        return genres

    @app.get("/app/{appid}/categories")
    def read_app_categories(appid: str, fuzzy: bool = True, db=db_dependency):
        return get_app_related_data(appid, db, models.Category, models.AppCategory, fuzzy)

    @app.get("/app/{appid}/genres")
    def read_app_genres(appid: str, fuzzy: bool = True, db=db_dependency):
        return get_app_related_data(appid, db, models.Genre, models.AppGenre, fuzzy)

    @app.get("/app/{appid}/tags")
    def read_app_tags(appid: str, fuzzy: bool = True, db=db_dependency):
        return get_app_related_data(appid, db, models.Tags, models.AppTags, fuzzy)

    @app.get("/app/{appid}")
    def read_app(appid: str, fuzzy: bool = True, db=db_dependency):
        return app_data_from_id_or_name(appid, db, fuzzy)

    @app.post("/app_input", response_class=HTMLResponse)
    def handle_form(request: Request, game_input: str = Form(...), db=db_dependency):
    ...
    @app.put("/app/{appid}/tag/{tagid}")
    def add_app_tag(appid: int, tagid: int, db=db_dependency):
        ...
        app_tag = db.query(models.AppTags).filter(models.AppTags.app_id == appid,
                                                  models.AppTags.tag_id == tagid).first()
        if app_tag:
    ...
    @app.delete("/app/{appid}/tag/{tagid}")
    def delete_app_tag(appid: int, tagid: int, db=db_dependency):
        if not os.environ.get("PYCHARM_HOSTED"):
            raise HTTPException(status_code=403,
                                detail="This endpoint is only available in the development environment.")
        elif not tagid or not appid:
    ...
    @app.put("/app/{appid}")
    def update_app(appid: int, name: str, db=db_dependency):
        app = db.query(models.App).filter(models.App.id == appid).first()
        if app:
            app.name = name
            db.commit()
    ...
    @app.post("/app")
    def add_app(name: str, appid: int, short_description: str = "", price: str = "", developer: str = "Seger",
                header_image: str = "", background_image: str = "", db=db_dependency):
        if not os.environ.get("PYCHARM_HOSTED"):
            raise HTTPException(status_code=403,
                                detail="This endpoint is only available in the development environment.")
        ...
        app = models.App(name=name, id=appid, short_description=short_description, price=price, developer=developer,
    ...
    def delete_app(appid: int, db=db_dependency):
        if not os.environ.get("PYCHARM_HOSTED"):
            raise HTTPException(status_code=403,
                                detail="This endpoint is only available in the development environment.")
    ...
    def app_data_from_id_or_name(app_id_or_name: str, db=db_dependency, fuzzy: bool = True):
        app = None
    ...
    @app.get("/apps/developer/{target_name}")
    def get_developer_games(target_name: str, fuzzy: bool = True, all_fields: bool = False, db=db_dependency):
        ...
        if similar_developer:
            ...
            try:
                ...
                raise HTTPException(status_code=404,
                                    detail=f"(AttributeError) No games found for developer {developer}")
            if games:
                return games
    ...
    def most_similar_named_developer(target_name: str, db=db_dependency):
        ...
        if most_similar_dev:
            print(
                f"Most similar developer: {most_similar_dev} with similarity: {similarity}. For target: {target_name}")
            return most_similar_dev.developer

        return None

    def most_similar_named_app(target_name: str, db=db_dependency):
    ...
    def find_similar_named_apps(target_name: str, db=db_dependency):
        ...
        for app in apps:
            ...
            if levenshtein_sim >= TRESHOLD or jaccard_sim >= JACCARD_TRESHOLD:
                similar_apps.append(
                    {"id": app.id, "name": app.name, "similarity": round(max(levenshtein_sim, jaccard_sim), 2)})

        return sorted(similar_apps, key=lambda x: x["similarity"], reverse=True)

    @app.get("/apps/tag/{target_name}")
    def get_apps_based_on_tag_name(target_name: str, fuzzy: bool = True, all_fields: bool = False, db=db_dependency):
